-- =====================================================
-- MIGRATION: Add is_pre_production column to projects
-- When true, project appears in "Pre-Production" section (between Building and Live)
-- =====================================================

-- Add is_pre_production column with default false
ALTER TABLE projects ADD COLUMN IF NOT EXISTS is_pre_production BOOLEAN NOT NULL DEFAULT false;

-- Create partial index for efficient filtering
CREATE INDEX IF NOT EXISTS idx_projects_is_pre_production ON projects(is_pre_production) WHERE is_pre_production = true;

-- Add descriptive comment
COMMENT ON COLUMN projects.is_pre_production IS 'When true, project appears in Pre-Production section on projects page (between Building and Live)';

-- =====================================================
-- Update get_project_stats to include is_pre_production
-- =====================================================
...
BEGIN
  ...
  GROUP BY
    ...
    p.is_live, p.is_finished, p.is_building, p.is_pre_production, p.logo_url, p.metadata
  ORDER BY
    p.created_at DESC;
END;
