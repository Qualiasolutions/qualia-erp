-- Allow audited admin force-close events to be written to activity_log.
-- The action is inserted by app/actions/work-sessions.ts:adminOverrideClockOut.

ALTER TABLE public.activity_log DROP CONSTRAINT IF EXISTS activity_log_action_type_check;

ALTER TABLE public.activity_log ADD CONSTRAINT activity_log_action_type_check
  CHECK (action_type IN (
    'phase_submitted',
    'phase_approved',
    'phase_changes_requested',
    'phase_started',
    'phase_completed',
    'phase_review_requested',
    'task_completed',
    'comment_added',
    'file_uploaded',
    'status_changed',
    'client_invited',
    'code_push',
    'deployment',
    'feature_request',
    'session_admin_overrode'
  ));
