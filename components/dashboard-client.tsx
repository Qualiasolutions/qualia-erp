...
function TaskItem({
  ...
  return (
    <div className="group flex items-center gap-4 rounded-xl border border-border bg-muted/30 p-4 transition-all hover:border-primary/30 hover:bg-muted/50 hover:shadow-glow-sm">
      <button
...
function ProjectItem({ project }: { project: DashboardProject }) {
  return (
    <Link
      ...
      className="group flex items-center justify-between rounded-xl border border-border bg-muted/30 p-4 transition-all duration-200 ease-premium hover:-translate-y-1 hover:border-primary/30 hover:bg-muted/50 hover:shadow-sm"
    >
      <div className="min-w-0 flex-1">
...
export function DashboardClient({
  ...
  return (
    <div className="min-h-screen bg-background selection:bg-primary/20">
      ...
      {/* Header */}
      <div className="relative border-b border-border bg-muted/10 px-8 py-12 backdrop-blur-md">
          ...
          <div className="flex items-center gap-4">
            <div className="flex -space-x-2">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="ease-out-quart flex h-10 w-10 items-center justify-center rounded-full border-2 border-background bg-card text-xs font-bold text-muted-foreground transition-transform duration-200 group-hover:scale-105"
                >
                  {String.fromCharCode(64 + i)}
                </div>
              ))}
              <div className="ease-out-quart flex h-10 w-10 items-center justify-center rounded-full border-2 border-background bg-primary/20 text-xs font-bold text-primary transition-transform duration-200 group-hover:scale-105">
                +12
              </div>
            </div>
          </div>
      ...
      <div className="relative mx-auto max-w-7xl px-8 py-12">
        <div className="grid gap-12 lg:grid-cols-12">
          ...
          <div className="space-y-12 lg:col-span-7">
            ...
            <section className="group relative">
              ...
              <div className="relative rounded-[1.5rem] border border-border bg-card/40 p-8 shadow-2xl backdrop-blur-sm">
                <div className="mb-8 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="hidden rounded-full border border-border bg-muted/30 px-4 py-1.5 text-xs font-bold text-muted-foreground sm:inline-flex">
                ...
                {tasks.length > 0 ? (
                  <div className="space-y-4">
                    ...
                    {tasks.length > 6 && (
                      <Link
                        href="/projects"
                        className="flex w-full items-center justify-center gap-2 rounded-xl border border-border py-4 text-sm font-bold text-muted-foreground transition-all hover:border-primary/20 hover:bg-muted/30 hover:text-foreground"
                      >
                  ...
                  <div className="rounded-2xl border border-dashed border-border bg-muted/20 py-16 text-center">
                    ...
                    <button className="mt-8 rounded-xl border border-border bg-muted/30 px-6 py-3 text-sm font-bold text-foreground transition-colors hover:bg-muted/50">
                      CREATE NEW TASK
                    </button>
                  </div>
                )}
            ...
            {pendingTasks.length > 0 && (
              <section className="rounded-[1.5rem] border border-border bg-card/40 p-8 backdrop-blur-sm">
          ...
          <div className="space-y-12 lg:col-span-5">
            <section className="rounded-[1.5rem] border border-border bg-card/60 p-8 shadow-xl backdrop-blur-md">
              <div className="mb-10 flex items-center justify-between">
                ...
                <Link
                  href="/projects"
                  className="flex h-10 w-10 items-center justify-center rounded-xl border border-border bg-muted/30 text-muted-foreground transition-all hover:border-primary/30 hover:text-primary"
                >
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="rounded-[1.5rem] border border-border bg-muted/20 p-6 transition-colors hover:bg-muted/30">
        ...
        <div className="mt-16 text-center">
          <div className="inline-flex items-center gap-3 rounded-full border border-border bg-muted/20 px-6 py-3 text-sm font-medium text-muted-foreground">
            <span>Press</span>
