    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center justify-between border-b border-border/40 px-5 py-4">
        ...
        <div className="flex items-center gap-1 rounded-lg bg-muted/50 p-1">
          {[1, 2, 3, 4].map((i) => (
            <Skeleton key={i} className="h-7 w-10 rounded-md bg-muted/50" />
          ))}
        </div>
      </div>
...
function TaskItemSkeleton({ delay = 0 }: { delay?: number }) {
  return (
    ...
    >
      <Skeleton className="mt-0.5 h-5 w-5 shrink-0 rounded-md bg-muted" />
      ...
      <Skeleton className="h-6 w-6 rounded-md bg-muted/50" />
    </div>
  );
}
...
function MeetingItemSkeleton({ delay = 0 }: { delay?: number }) {
  return (
    ...
    >
      ...
      <Skeleton className="h-8 w-14 rounded-lg bg-muted" />
    </div>
  );
}
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        <div className="flex items-center justify-between border-b border-border/40 px-5 py-4">
          <div>
            ...
            <Skeleton className="mt-1.5 h-3 w-20 bg-muted" />
          </div>
      ...
      <div className="border-t border-border/40">
        <div className="px-5 py-4">
          ...
          <Skeleton className="mt-1.5 h-3 w-16 bg-muted" />
        </div>
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {!compact && (
          <>
            <div className="mt-2 flex items-center gap-2">
              <Skeleton className="h-1.5 flex-1 rounded-full bg-muted" />
...
export function DashboardSkeleton() {
  return (
    <div className="flex h-screen flex-col overflow-hidden bg-background">
      {/* Header Skeleton */}
      <header className="flex h-16 shrink-0 items-center justify-between border-b border-border/40 bg-background/80 px-5 backdrop-blur-xl lg:px-8">
        ...
        <div className="flex items-center gap-2">
          <Skeleton className="hidden h-9 w-24 rounded-lg bg-muted lg:block" />
      ...
      <main className="min-h-0 flex-1 overflow-hidden p-4 lg:p-6">
        <div className="mx-auto grid h-full max-w-[1920px] grid-cols-1 gap-5 lg:grid-cols-3">
          {/* Tasks Card */}
          <div className="group relative flex flex-col overflow-hidden rounded-2xl bg-card/80 shadow-2xl ring-1 ring-white/[0.08] backdrop-blur-xl">
