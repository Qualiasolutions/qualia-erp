...
function PhaseRow({
  ...
  const completed = phase.plans_completed || 0;
...
export function ProjectWorkflow({
  ...
  const fetchData = useCallback(async () => {
    try {
      const phasesData = await getProjectPhases(projectId);
  ...
  const handleDeletePhase = (phaseId: string) => {
    setConfirmDialog({
      title: 'Delete phase?',
      description: 'This will delete this phase. This action cannot be undone.',
      confirmLabel: 'Delete',
      action: () => {
        startTransition(async () => {
  ...
  if (isDrilledIn) {
    ...
    const plansDone = activePhase?.plans_completed || 0;
    const plansTotal = activePhase?.plan_count || 0;
      ...
      <div className={cn('flex flex-col', className)}>
        ...
        <div className="scrollbar-thin min-h-0 flex-1 overflow-y-auto">
          <div className="px-5 py-3">
            {/* Framework-sourced item breakdown (from each phase PLAN.md) */}
            {activePhase && <PhaseItemsList phaseId={activePhase.id} />}
          </div>
        </div>
